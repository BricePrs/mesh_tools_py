    ...
    pub fn new(vertex_source_file: &str, fragment_source_file: &str) -> Self {
        ...
        unsafe {
            ...
            if success == 0 {
                ...
                gl::GetShaderInfoLog(shader_program, 
                    1024, 
                    &mut error_size,
                    error.as_mut_ptr().cast()
                );
                error.set_len(error_size.try_into().unwrap());
                panic!("ERROR::PROGRAM::COULD_NOT_LINK: {}", String::from_utf8_lossy(&error));
            }
    ...
    fn create_shader(file_path: &str, shader_type: GLenum) -> GLuint {
        ...
        unsafe {
            ...
            if success == 0 {
                ...
                error.set_len(error_size.try_into().unwrap());
                panic!("Error : Could not compile shader : {}", String::from_utf8_lossy(&error));
            }
        }
